...
1. **What just happened in Part 1.** No map, no commands, no conventions. It did the only thing it could: pattern-match from training data onto a codebase it hadn't read. That's not a model failure. That's a context failure, and it was ours.  Note that if your codebase follows good engineering practices, poor prompting works MUCH better.  If it's a fractured codebase with differences all over the place and no great way to run it, you're gonna have a bad time
...
3. **Why fresh sessions.** Long conversations accumulate wrong turns and cost. A session that starts from written artifacts and the codebase only.
